 ...
 */
function clickedCardIndex(event) {
 ...
 */
function applyBoardLayout(board, config) {
 ...
 */
function showMatch(finished) {
    syncCards();
    syncTopbar();
    if (finished)
        window.setTimeout(showResult, FINISH_DELAY);
}
/**
 * Draws a missed turn.
 *
 ...
 */
function syncCard(button, card, index) {
 ...
 */
function assertNever(outcome) {
    throw new Error(`Unhandled flip outcome: ${JSON.stringify(outcome)}`);
}
